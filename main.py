...
def run_planner_cycle():
    ...
    for i, row in enumerate(records, start=2):
        ...
        if pub_time and now >= pub_time:
            ...
            tg_id = str(row.get('TG'))
            tg_status = row.get('Статус TG')
            ok_id = str(row.get('OK'))
            ok_status = row.get('Статус OK')
            vk_id = str(row.get('VK'))
            vk_status = row.get('Статус VK')

            if tg_id == "1" and tg_status not in ["Опубликовано", "Удалено"]:
                logger.info(f"[{post_id}] Публикуем в Telegram...")
                res = tg.send_to_telegram(text, img)
                if res:
                    cells_to_update.extend(
                        [Cell(i, idx_tg_s, "Опубликовано"),
                         Cell(i, idx_tg_id, str(res))]
                         )
                    row['Статус TG'] = "Опубликовано"
                    row['message_id'] = str(res)

            if ok_id == "1" and ok_status not in ["Опубликовано", "Удалено"]:
                logger.info(f"[{post_id}] Публикуем в Одноклассники...")
                res = ok.publish_to_ok(text, img)
                if res:
                    cells_to_update.extend(
                        [Cell(i, idx_ok_s, "Опубликовано"),
                         Cell(i, idx_ok_id, str(res))]
                         )
                    row['Статус OK'] = "Опубликовано"
                    row['ok_post_id'] = str(res)

            if vk_id == "1" and vk_status not in ["Опубликовано", "Удалено"]:
                logger.info(f"[{post_id}] Публикуем в VK...")
                res = vk.publish_to_vk(text, img)
                if res:
                    cells_to_update.extend(
                        [Cell(i, idx_vk_s, "Опубликовано"),
                         Cell(i, idx_vk_id, str(res))])
                    row['Статус VK'] = "Опубликовано"
                    row['vk_post_id'] = str(res)

        is_del = str(
            row.get('Удалить автоматически?')
            ).strip().upper() in ["TRUE", "1"]
        
        del_time = parse_datetime(row.get('Удалить'))

        if is_del and del_time and now >= del_time:
            if tg_status == "Опубликовано" and row.get('message_id'):
                logger.info(f"[{post_id}] Удаляем из Telegram...")
                if tg.delete_message(row.get('message_id')):
                    cells_to_update.extend(
                        [Cell(i, idx_tg_s, "Удалено"),
                         Cell(i, idx_tg_id, "")]
                         )

            if ok_status == "Опубликовано" and row.get('ok_post_id'):
                logger.info(f"[{post_id}] Удаляем из Одноклассников...")
                if ok.delete_ok_post(row.get('ok_post_id')):
                    cells_to_update.extend(
                        [Cell(i, idx_ok_s, "Удалено"),
                         Cell(i, idx_ok_id, "")])

            if vk_status == "Опубликовано" and row.get('vk_post_id'):
                logger.info(f"[{post_id}] Удаляем из VK...")
                if vk.delete_vk_post(row.get('vk_post_id')):
                    cells_to_update.extend(
                        [Cell(i, idx_vk_s, "Удалено"),
                         Cell(i, idx_vk_id, "")])
